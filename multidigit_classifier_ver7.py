...
def extract_digits_from_image(image_path, debug=False):
    ...
    for x, y, w, h in digit_boxes:
        # Extract Digit with Padding and Squaring
...
def extract_digit_with_padding_and_squaring(img, x, y, w, h):
    """
    Extracts a digit from the image, adds padding, and places it in a square.
    ...
    Returns:
        np.ndarray: The squared digit image with padding, or None if error.
    """
    padding_percent = 0.2
    padding_x = int(w * padding_percent)
    padding_y = int(h * padding_percent)
...
def merge_and_adjust_overlapping_boxes(boxes):
    """
    Merges and adjusts overlapping bounding boxes to prevent overlap.
    """
    if len(boxes) <= 1:
        return boxes
...
def main():
    ...
    for image_path in image_paths:
        ...
        digit_images, digit_boxes = extract_digits_from_image(image_path, debug=False)
